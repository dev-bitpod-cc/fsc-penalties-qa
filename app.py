...
Version: 1.3.0 - 新增查詢結果概述功能 (2025-11-20)
  - ✨ 新增概述區塊：在列出具體案件前先總結整體情況
  - 📊 概述包含：案件數量、違規類型、時間分布、裁罰金額範圍
  - 🎯 改善資訊架構：概述 → 具體案件 → 參考來源
  - 📝 優化 system_instruction，要求 AI 提供結構化回答
  - 參考 Sanction-Deploy 的呈現方式進行優化
...
3. **回答格式要求**（關鍵）：
   - **第一部分：概述** - 先提供 2-3 句簡短的總結
     - 總共找到幾筆相關案件
     - 主要的違規類型或共同特徵
     - 時間分布或趨勢觀察（如果相關）
     - 裁罰金額範圍（如果相關）
   - **第二部分：具體案件** - 列出前 3-5 筆最相關的案件詳細資訊
...
根據查詢結果，資料庫中共找到 X 筆相關裁罰案件。這些案件主要涉及 [違規類型]，集中在 [時間範圍]，裁罰金額從 [最小金額] 到 [最大金額] 不等。以下列出最具代表性的案件：
